...
### Период данных в одном DAG Run

Airflow DAG `etl_variant_09` запускается по расписанию:
```text
*/5 * * * *
```
То есть scheduler создаёт новый DAG Run каждые 5 минут. При этом 5 минут — это **частота проверки и запуска пайплайна**, а не период бизнес‑данных.

Один DAG Run обрабатывает период, который определяется режимом запуска:

* **`full`**:
  * один DAG Run обрабатывает **всю доступную историю** показателя из World Bank API;
  * для текущего набора GDP это годовой ряд от минимального доступного года до последнего года, который вернул API;
  * расписание Airflow не режет данные на 5‑минутные интервалы.

* **`incremental`**:
  * один DAG Run обрабатывает только записи **строго новее `watermark`** из `data/state/state.json`;
  * фактический период запуска — `(last_date, max_available_date]`;
  * после успешной загрузки `watermark` сдвигается на максимальную дату/год, реально попавшую в обработанный набор.

Такой подход нужен потому, что источник World Bank отдаёт макроэкономические наблюдения по годам. Поэтому период данных задаётся годами наблюдений (`year`), а не временем запуска DAG.
...
Дубли могут появиться, если один и тот же период будет обработан повторно: например, при ручном перезапуске DAG Run, retry после временной ошибки или повторном `incremental`‑запуске до обновления `watermark`.

Защита строится на трёх уровнях:

1. **Слой `transform`**
   * после нормализации выполняется `drop_duplicates()`;
   * это убирает полностью одинаковые строки внутри одного raw‑ответа.

2. **Бизнес‑ключ**
   * для годовой витрины GDP ключом является:
     ```text
     (year, country_iso3)
     ```
   * для одной страны, одного индикатора и одного года должно быть ровно одно значение;
   * повторная строка с тем же ключом считается дублем.

3. **Загрузка в БД**
   * в `full`‑режиме таблицы пересоздаются, поэтому повторный запуск не увеличивает количество строк;
   * в `incremental`‑режиме безопасный вариант — делать `upsert` по бизнес‑ключу или предварительно удалять/заменять строки за обрабатываемый период;
   * если используется простой `append`, перед загрузкой нужно дедуплицировать данные по `(year, country_iso3)` и контролировать отсутствие дублей SQL‑проверкой.

SQL‑проверка дублей для `russia_gdp`:
```sql
SELECT year, country_name, COUNT(*)
FROM russia_gdp
...
#### Почему `retry` безопасен

`retry` безопасен, потому что повторяется не произвольное изменение данных, а тот же самый детерминированный ETL‑запуск для того же логического периода:

* **Extract** повторно запрашивает тот же источник и сохраняет новый raw‑файл с новым `timestamp`; старый raw‑файл не перезаписывается.
* **Transform** строит normalized‑таблицу из конкретного raw‑файла, указанного в `state["timestamp"]`, и удаляет дубли внутри набора.
* **Mart** пересчитывается из normalized‑слоя, а не дописывается вручную строка за строкой.
* **Load** в `full`‑режиме пересоздаёт целевые таблицы, поэтому повтор не создаёт удвоение строк.
* **Load** в `incremental`‑режиме должен использовать бизнес‑ключ `(year, country_iso3)` для `upsert`/дедупликации; тогда повторная обработка того же периода обновит существующие строки, а не добавит копии.
* **`watermark` обновляется только после успешного завершения всех этапов**, поэтому при падении DAG Run данные не считаются загруженными. Повторный запуск снова возьмёт тот же период и загрузит его безопасно.

Итог: retry не приводит к потере данных, потому что `watermark` не сдвигается до успешного `load`, и не приводит к дублям, потому что результат защищён пересозданием таблиц в `full` или бизнес‑ключом в `incremental`.
...
Вот более короткая версия для `week‑7` в том же стиле:

***

## Чекпоинт: `week‑7` (визуализации и выводы)

### Цели

* Создать `notebooks/week7_viz.ipynb` с визуализациями по витрине `russia_gdp`.
* Построить три графика:
  * временной ряд GDP по годам;
  * распределение значений GDP;
  * ранжирование топ‑5 и худших 5 лет по GDP.
* Сформулировать три текстовых вывода, привязанных к графикам.
* Сохранить графики в `docs/figures/*` и обеспечить полную воспроизводимость Ноутбука.
***

### Ноутбук `week7_viz.ipynb`

* В первой ячейке загружается `mart_yearly` из `data/mart/variant_09/mart_yearly_*.csv`, проверяются `df.shape`, колонки (`year`, `gdp`, `country_name`), типы и `NaN`.
* `year` приводится к `datetime` и используется в трёх последующих графиках.

***

### Три вопроса к данным

1. **Динамика по годам**: есть ли резкий рост/спад, пики, периоды стабилизации?  
   → соответствует **График 1 (временной ряд)**.

2. **Распределение**: какие значения встречались чаще или реже?  
   → соответствует **График 2 (density / histogram)**.

3. **Ранжирование**: какие годы — лидеры, какие — отстающие по GDP?  
   → соответствует **График 3 (bar chart топ‑5 и худшие 5)**.
...
Пайплайн теперь включает полный цикл:

```text
Extract → Transform → Mart → week7_viz.ipynb (визуализации)
```

Реализовано:
* три содержательных графика по одной витрине;
* три связанных с ними вывода;
* воспроизводимые `PNG`‑изображения в `docs/figures/`.

Теперь работа демонстрирует не только корректный ETL, но и **аналитически читаемые интерпретации** данных.

***

## Чекпоинт: `week-14` (LLM-summary и финальная проверяемость)

### Цели

* Собрать проект как воспроизводимый мини-продукт: pipeline, mart, DQ, BI и LLM-сводка.
* Использовать LLM только для интерпретации уже рассчитанных метрик.
* Исключить ситуацию, когда LLM придумывает числа, поля или выводы без опоры на данные.
* Зафиксировать использование LLM в `docs/LLM_Usage_Log.md`.

***

### Финальный сценарий запуска
...
Эта команда обновляет raw, normalized, mart, загружает данные в PostgreSQL, пересоздает DQ-отчеты и строит LLM-сводку
по уже готовым агрегатам.

Отдельный ручной запуск DQ и LLM-summary остается доступен:
...
### LLM как отдельный слой

LLM-шаг реализован отдельно:
...
* `data/mart/variant_09/mart_yearly_*.csv`;
* `data/mart/variant_09/mart_stats_*.csv`;
* `docs/dq/dq_report.json`.

Raw JSON, большие таблицы и приватные данные в LLM не передаются.
...
* идентичность датасета: World Bank GDP, Russia, variant 09;
* период данных;
* количество строк в витрине;
* latest GDP;
* delta за 10 лет;
* average за последние 10 лет;
* max/min GDP;
* общий DQ-статус;
* запрет на расчет новых чисел и выдумывание метрик.

***

### Проверяемость

Все числа в `docs/llm/summary.md` берутся из mart-слоя или DQ-отчета. Скрипт дополнительно проверяет, что ключевые
значения из контекста присутствуют в итоговой сводке.

Если используется реальный API-запрос, ключ хранится только в `.env`, а `.env` исключен из Git через `.gitignore`.
...
К финалу проект содержит:

* воспроизводимый pipeline;
* mart-слой с агрегированными KPI;
* DQ-отчеты в `docs/dq/`;
* BI-артефакты в `docs/bi/`;
* LLM-сводку в `docs/llm/summary.md`;
* LLM-log с записью о цели, контексте, промпте, проверке и результате.
